...
// Leagues that get a full sync: real standings + real squads + real season stats.
const FULL_LEAGUES = [
  ...
  { id: 64, name: 'Scottish Premiership' },
...
// Leagues that only get real standings + real upcoming fixtures (no squad/player sync,
// to keep the sync inside Vercel's function time limit).
const TABLE_LEAGUES = [
  { id: 42, name: 'Champions League' },
  { id: 73, name: 'Europa League' },
  { id: 10216, name: 'Conference League' },
  { id: 135, name: 'Greek Super League' },
  { id: 69, name: 'Swiss Super League' },
  { id: 46, name: 'Danish Superliga' },
  { id: 59, name: 'Eliteserien' },
  { id: 67, name: 'Allsvenskan' },
  ];

// Friendlies have no standings table — fixtures only.
const FIXTURES_ONLY_LEAGUES = [
  { id: 489, name: 'Club Friendlies' },
  { id: 114, name: 'International Friendlies' },
  ];
...
async function fetchLeaguePageSmart(id) {
    try {
          return await fetchJson(`https://www.fotmob.com/api/data/leagues?id=${id}&season=2025/2026`);
    } catch (e) {
          return await fetchJson(`https://www.fotmob.com/api/data/leagues?id=${id}`);
    }
}

function extractTable(leaguePage, league) {
    const table = leaguePage.table?.[0]?.data?.table?.all || [];
    return table.map((t) => {
          ...
          return {
                  ...
                  standings: { position: t.idx, played: t.played, wins: t.wins, draws: t.draws, losses: t.losses, goalsFor: parts[0], goalsAgainst: parts[1], points: t.pts },
          };
    });
}

function extractFixtures(leaguePage, league) {
    const all = leaguePage.fixtures?.allMatches || [];
    const upcoming = all.filter((m) => m.status && !m.status.finished && !m.status.cancelled && m.status.utcTime);
    upcoming.sort((a, b) => new Date(a.status.utcTime) - new Date(b.status.utcTime));
    return upcoming.slice(0, 100).map((m) => ({
          id: m.id,
          league: league.name,
          leagueId: league.id,
          round: m.round || null,
          date: m.status.utcTime,
          home: clubName(m.home?.shortName || m.home?.name || 'TBD'),
          away: clubName(m.away?.shortName || m.away?.name || 'TBD'),
...
async function syncLeagueFull(league) {
    const playersById = new Map();

  let leaguePage = await fetchLeaguePageSmart(league.id);
  ...
  const clubs = extractTable(leaguePage, league);
    const fixtures = extractFixtures(leaguePage, league);
...
async function syncLeagueLite(league) {
    const leaguePage = await fetchLeaguePageSmart(league.id);
    const clubs = extractTable(leaguePage, league);
    const fixtures = extractFixtures(leaguePage, league);
    return { clubs, players: [], fixtures };
}
...
export default async function handler(req, res) {
  ...
  try {
      ...
      const fullResults = await pool(FULL_LEAGUES, 4, async (league) => {
              try {
                        const r = await syncLeagueFull(league);
                        console.log(`[full-database] ${league.name}: ${r.clubs.length} clubs, ${r.players.length} players, ${r.fixtures.length} fixtures`);
                        return r;
              } catch (e) {
      ...
      const liteResults = await pool([...TABLE_LEAGUES, ...FIXTURES_ONLY_LEAGUES], 6, async (league) => {
              try {
                        const r = await syncLeagueLite(league);
                        console.log(`[full-database] ${league.name} (lite): ${r.clubs.length} clubs, ${r.fixtures.length} fixtures`);
                        return r;
              } catch (e) {
                        console.error(`[full-database] ${league.name} (lite) failed:`, e.message);
                        return null;
              }
      });

      [...fullResults, ...liteResults].filter(Boolean).forEach((r) => {
              allClubs.push(...r.clubs);
      ...
      console.log(`[full-database] Synced: ${merged.players.length} players, ${merged.clubs.length} clubs, ${merged.fixtures.length} fixtures`);

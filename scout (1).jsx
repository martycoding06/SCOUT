...
/* ============================================================
   SCOUT. — real 2025/26 player data (FBref, Big-5 leagues)
   ------------------------------------------------------------
   Every stat shown is REAL, sourced directly from four FBref
   2025/26 exports: Standard (goals, assists, minutes, matches,
   cards, penalties), Goalkeeping (saves, goals against, save%,
   clean sheets, W/D/L), Misc (tackles won, interceptions), and
   Shooting (shots, shots on target).
   Nothing is simulated, invented, or spread across fake matches.
   The only non-FBref number is the "Scout Score" — a formula
   Claude computed from real per-90 output — and it is labeled
   as an estimate everywhere it appears, never as a raw stat.
   Prop probabilities are a Poisson model applied to each
   player's real season rate (e.g. real goals per game) — a
   projection built from real data, not a fabricated hit rate.
   ============================================================ */

const clamp = (v, lo, hi) => Math.max(lo, Math.min(hi, v));

const REAL= function useSubAgentSync() {
...
/* ------------------------ aggregation ------------------------
   Every field here comes straight from the four FBref CSVs
...
/* power table — derived entirely from real player data:
   each club ranked by its minutes-weighted team points-per-match
   (FBref Playing Time), with real squad goals and appearances.
   This is NOT the official league table. */
const TABLE = {};
LEAGUES.forEach((lg) => {
  const clubs = CLUB_NAMES.filter((c) => CLUBS[c].league === lg);
  ...
  return (
    <div>
      ...
      <div className="dhead">
        <div>
          ...
          <div className="section-note" style={{ margin: "8px 0 0" }}>
            Scout Score (estimated, not an FBref stat) <span className="mono" style={{ color: "var(--amber)" }}>{p.rating.toFixed(2)}</span>
          </div>
        </div>
        <div className="dbtns">
      ...
      {p.pos !== "GK" && p.agg.ppm != null && (
        <div className="panel" style={{ marginBottom: 14 }}>
          <div className="ptitle">Team impact while on the pitch <span className="mono" style={{ fontSize: 11, color: "var(--muted)" }}>FBref Playing Time</span></div>
          <div className="statgrid">
  ...
  return (
    <div>
      ...
      <div className="panel">
        ...
        <div className="section-note" style={{ marginTop: 10, marginBottom: 0 }}>
          Ranked by each squad's real FBref team points-per-match, weighted by every player's real minutes. Squad goals and appearances are real season totals; this ordering is a derived power ranking, not the league's official table.
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ maxWidth: 760, paddingTop: 16 }}>
      <div className="panel" style={{ marginBottom: 14 }}>
        ...
        <div className="section-note" style={{ fontSize: 13 }}>
          ...
          from five FBref exports: <b>Standard</b>, <b>Goalkeeping</b>, <b>Misc</b>, <b>Shooting</b>, and <b>Playing Time</b>. Every stat below is real —
        ...
        <table>
          ...
          <tbody>
            <tr><td style={{ color: "var(--green)" }}>Goals, assists, penalties</td><td style={{ color: "var(--muted)" }}>FBref Standard</td></tr>
            <tr><td style={{ color: "var(--green)" }}>Minutes, matches, starts</td><td style={{ color: "var(--muted)" }}>FBref Standard</td></tr>
            <tr><td style={{ color: "var(--green)" }}>Yellow &amp; red cards</td><td style={{ color: "var(--muted)" }}>FBref Standard</td></tr>
            <tr><td style={{ color: "var(--green)" }}>Tackles won &amp; interceptions</td><td style={{ color: "var(--muted)" }}>FBref Misc</td></tr>
            <tr><td style={{ color: "var(--green)" }}>Shots &amp; shots on target</td><td style={{ color: "var(--muted)" }}>FBref Shooting</td></tr>
            <tr><td style={{ color: "var(--green)" }}>Team points/match, on-pitch goals, +/-, on/off diff</td><td style={{ color: "var(--muted)" }}>FBref Playing Time</td></tr>
            <tr><td style={{ color: "var(--green)" }}>Saves, goals against, save%, clean sheets, W-D-L</td><td style={{ color: "var(--muted)" }}>FBref Goalkeeping</td></tr>
          </tbody>
        </table>
      </div>
      <div className="panel" style={{ marginBottom: 14 }}>
        <div className="ptitle">The two things that aren't raw FBref numbers</div>
        <div className="section-note" style={{ fontSize: 13 }}>
          ...
          everywhere it appears and is never presented as an FBref stat. It only affects default sort order in Players.
        </div>
        <div className="section-note" style={{ fontSize: 13 }}>
      ...
      <div className="section-note">
        ...
        the three real CSVs above, except the two items explicitly labeled "estimated" here.
      </div>
    </div>
  );
}
    ...
    synced {getTimeAgo(lastSync)}
      ...
      <main className="wrap" style={{ paddingTop: tab === "players" && !openId ? 4 : 0 }}>
        ...
        <footer className="footer">
          <b>Data:</b> real 2025/26 season stats from FBref (Big-5 leagues). Goals, assists, minutes, matches, cards, tackles, interceptions, shots, shots on target, on-pitch team goal differential, and all goalkeeping totals are real — nothing is simulated. Non-FBref numbers are clearly labeled: the "Scout Score" (an estimate), player prop probabilities (Poisson projections from real season rates), the illustrative matchday (generated pairings; win/goal probabilities are a Poisson model over each club's strength index), and the power table (real minutes-weighted points-per-match, not official standings). Prices shown are indicative, never live market odds.<br />
